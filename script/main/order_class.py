...
class OrderProcessFunctionality:
    ...
    def filter_and_sort_data(self, product_id: str) -> pd.DataFrame:
        self.__df = self.__order_df[self.__order_df['product_code'] == product_id]
        self.__df = self.__df.sort_values(by=['order_date']).reset_index(drop=True)
        #print(len(self.df))
        return self.__df
...
if __name__ == "__main__":
    ...
    print(df1)
    index_num = o.index_identify(df1)
    print(index_num)
